...
public class Main extends AppCompatActivity {
    ...
    RecyclerView recyclerMoviesSearch;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerMoviesSearch = (RecyclerView) findViewById(R.id.recyclerMoviesSearch);
    ...
    private void upComingMovies(){
        ...
        movieService.upcomingMovies(language, upComingPage).enqueue(new Callback<Films>() {
            @Override
            public void onResponse(Call<Films> call, Response<Films> response) {
                moviesList.addAll(response.body().getResults());
                adapter = new Adapter(moviesList, genresList);
    ...
    private void moviesList(){
        ...
        recyclerMovies.setVisibility(View.VISIBLE);
        searchLayout.setVisibility(View.INVISIBLE);
        movieService.listMovies(language,popularPage).enqueue(new Callback<Films>() {
            @Override
            public void onResponse(Call<Films> call, Response<Films> response) {
                moviesList.addAll(response.body().getResults());
                adapter = new Adapter(moviesList, genresList);
    ...
    private void genresList(){
        genresList.clear();
        movieService.listGenres().enqueue(new Callback<Catégories>() {
            @Override
            public void onResponse(Call<Catégories> call, Response<Catégories> response) {
                genresList = response.body().getGenres();
            }

            @Override
            public void onFailure(Call<Catégories> call, Throwable t) {
                System.out.println("Error " + t.getMessage());
    ...
    private void searchMovies(){
        moviesList.clear();
        movieService.moviesSearch(searchMovie.getText().toString(),language).enqueue(new Callback<Films>() {
            @Override
            public void onResponse(Call<Films> call, Response<Films> response) {
                moviesList = response.body().getResults();
                Adapter adapter = new Adapter(moviesList, genresList);
                recyclerMoviesSearch.setAdapter(adapter);
                recyclerMoviesSearch.setLayoutManager(searchLayoutManager);
